/-
  ...
  for: every axiom of `RAlt ν` (`ν ≥ 1`) has cut rank below `ω · ν`
  (`Theory.lean`'s `lvlOf_emb_lt_of_mem_RAlt`, `Evaluate.lean`'s `lvlOf_evR`,
  and `Rank.lean`'s `rank_lt_block_of_level`, composed).  This is what lets
  a proof from `RAlt ν` be replayed (`Ramified/Embed.lean`'s `replayR`) at a
  cut rank below `ω · ν`, which predicative cut elimination removes one block
...
(`ν ≥ 1`), evaluated and embedded, has cut rank strictly below `ω · ν`. -/
theorem rank_evR_emb_lt_of_mem_RAlt {ν : Lv} (hν : 1 ≤ ν) {σ : Sentence LRA} (h : σ ∈ RAlt ν) :
  ...
  exact rank_lt_block_of_level hlvl'
